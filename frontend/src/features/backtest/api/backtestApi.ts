...
class BacktestApiService extends ApiClient {
   ...
   * @returns 주식 과거 데이터 리스트
   */
  async getStockHistoricalData(
    stockName: string,
    date: string
  ): Promise<Result<StockHistoricalDataResponse[], AppError>> {
    console.log(`🔍 백테스트용 주식 검색 API 호출: /api/v1/backtests/stocks/historical`);

    const result = await this.get<StockHistoricalDataResponse[]>(
      `/api/v1/backtests/stocks/historical?stockName=${encodeURIComponent(stockName)}&date=${date}`
    );

    if (result.success) {
      console.log('✅ 백테스트용 주식 검색 API 응답:', result.data);
    } else {
      console.error('❌ 백테스트용 주식 검색 API 에러:', result.error);
    }
  ...
  async getBacktestList(
    ...
    console.log(`📋 백테스트 목록 조회 API 호출: /api/v1/backtests`);

    const result = await this.get<PageResponse<BacktestListResponse>>(
      `/api/v1/backtests?page=${page}&size=${size}`
    );

    if (result.success) {
      console.log('✅ 백테스트 목록 조회 API 응답:', result.data);
    } else {
      console.error('❌ 백테스트 목록 조회 API 에러:', result.error);
    }
  ...
  async createBacktest(request: BacktestCreateRequest): Promise<Result<number, AppError>> {
    console.log(`🚀 백테스트 생성 API 호출: /api/v1/backtests`, request);

    const result = await this.post<number>('/api/v1/backtests', request);

    if (result.success) {
      console.log('✅ 백테스트 생성 API 응답:', result.data);
    } else {
      console.error('❌ 백테스트 생성 API 에러:', result.error);
    }
  ...
  async validateBacktest(request: BacktestCreateRequest): Promise<Result<BacktestValidationResponse, AppError>> {
    console.log(`✅ 백테스트 검증 API 호출: /api/v1/backtests/validate`, request);

    const result = await this.post<BacktestValidationResponse>('/api/v1/backtests/validate', request);

    if (result.success) {
      console.log('✅ 백테스트 검증 API 응답:', result.data);
    } else {
      console.error('❌ 백테스트 검증 API 에러:', result.error);
    }

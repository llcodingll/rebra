 ...
 */
export const useBuyOrder = ({ stockCode, stockName, onSuccess, onError }: UseBuyOrderProps) => {
  ...
  const mutation = useMutation({
    mutationFn: async (orderData: BuyOrderRequest) => {
      ...
      const result = await stockOrderApi.buyStock(stockCode, stockName, orderData);
  ...
  const buyStock = useCallback(
    (orderData: Omit<BuyOrderRequest, 'orderType' | 'accountId' | 'stockName'>) => {
      if (!accountId) {

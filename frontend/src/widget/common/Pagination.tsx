...
export default function Pagination({ 
  ...
  const renderPageNumbers = () => {
    ...
    for (let i = startPage; i <= endPage; i++) {
      if (i === currentPage) {
        pages.push(
          <button key={i} className={`${styles.pageNumber} ${styles.active}`}>
            {i}
          </button>
        );
      } else {
    ...
    if (endPage < totalPages) {
      if (endPage < totalPages - 1) {
        pages.push(
          <button key="ellipsis" className={styles.pageNumber}>
            {icons?.ellipsis ? (
              <img src={icons.ellipsis} alt="..." />
            ) : (
